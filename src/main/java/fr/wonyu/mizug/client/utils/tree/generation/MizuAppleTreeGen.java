package fr.wonyu.mizug.client.utils.tree.generation;
...
public class MizuAppleTreeGen extends WorldGenAbstractTree {

    private int minTreeHeight = 5;
    private int maxTreeHeight = 6;

    private IBlockState blockLog = MizuBlocks.APPLE_LOG.getDefaultState();
    private IBlockState blockLeaves = MizuBlocks.APPLE_LEAF.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false));


    public MizuAppleTreeGen(boolean notify) {
        super(notify);
    }
    ...
    public boolean generate(World worldIn, Random rand, BlockPos pos) {
        if ( !worldIn.isRemote ) {
            ...
            if (canGrow(worldIn, pos, maxGrowthHeight)) {
                worldIn.setBlockState(pos, blockLog);
                for (int i = 1; i <= maxGrowthHeight; i++) {
                    worldIn.setBlockState(pos.up(i), blockLog);
    ...
    private void spawnLeaves(World worldIn, BlockPos pos ) {
        if ( isReplaceable(worldIn, pos)) {
            worldIn.setBlockState(pos, blockLeaves);
        }
    }
}
