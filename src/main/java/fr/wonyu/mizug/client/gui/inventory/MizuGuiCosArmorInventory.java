...
public class MizuGuiCosArmorInventory extends InventoryEffectRenderer implements IRecipeShownListener
{

    public static final ResourceLocation texture = new ResourceLocation(References.MODID, "textures/gui/inventorycosm.png");
    ...
    private Map<Integer, MizuGuiCosArmorToggleButton> toggleButtons = Maps.newHashMapWithExpectedSize(4);

    public MizuGuiCosArmorInventory(Container container)
    {
        super(container);
        allowUserInput = true;
    }
    ...
    protected void actionPerformed(GuiButton button)
    {
        if (button.id == 10)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                MizuGuiCosArmorToggleButton t = toggleButtons.get(i);
                if (t == null)
                    continue;
                t.x = guiLeft + 97 + 18 * i;
                t.y = guiTop + 61;
    ...
    public void initGui()
    {
        ...
        recipeBook.func_194303_a(width, height, mc, widthTooNarrow, ((MizuContainerCosArmor) inventorySlots).craftMatrix);
        guiLeft = recipeBook.updateScreenPosition(widthTooNarrow, width, xSize);

       // recipeBookButton = new GuiButtonImage(10, guiLeft + 20, guiTop + 60, 20, 18, 178, 0, 19, INVENTORY_BACKGROUND);
        ...
        {
            int j = 3 - i;
            MizuGuiCosArmorToggleButton t = new MizuGuiCosArmorToggleButton(80 + j, guiLeft + 115 + 0 * i, 178 + i * 18, 5, 5, "");
            t.state = MizuG.proxy.getCosArmorInventoryClient(mc.player.getUniqueID()).isSkinArmor(j) ? 1 : 0;
            buttonList.add(t);
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException
    {
        ...
        {
            if (keyCode == MizuG.keyHandler.keyOpenCosArmorInventory.getKeyCode())
